...
const createRentalOrder = async (
  ...
  const orderItemsData = payload.items.map((item) => {
    const gear = gearItems.find((g: any) => g.id === item.gearItemId)!;
    const quantity = item.quantity ?? 1;
    if (gear.stock < quantity)
      throw new AppError(400, `Not enough stock for "${gear.name}"`);
...
const getRentalOrderById = async (id: string, userId: string, role: string) => {
  ...
  const isProvider = order.items.some(
    (i: any) => i.gearItem.providerId === userId,
  );
...
const updateOrderStatus = async (
  ...
  const isProvider = order.items.some(
    (i: any) => i.gearItem.providerId === providerId,
  );
  if (!isProvider)
    throw new AppError(403, "You do not manage any item in this order");
